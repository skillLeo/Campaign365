    ...
    return (
      <div className="flex items-center justify-center h-full fade-in">
        <Card className="p-10 text-center max-w-md">
          <div className="w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center bg-teal-50">
            <CheckCircle size={36} className="text-teal-500" />
          </div>
          <h2 className="text-xl font-bold text-slate-800 mb-2">Tenant Created!</h2>
          <p className="text-sm text-slate-500 mb-1">
            <strong>{form.party_name}</strong> is now live at
          </p>
          <p className="text-teal-600 font-mono text-sm mb-6">{form.subdomain}.campaign365.com</p>
          <p className="text-xs text-slate-400 mb-6">Login credentials have been emailed to {form.admin_email}</p>
  ...
  return (
    <div className="space-y-5 fade-in max-w-3xl mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-800">Onboard New Client</h1>
          <p className="text-sm text-slate-500">5-step setup wizard</p>
        </div>
      </div>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={s.id} className="flex items-center flex-1 last:flex-none">
                ...
                {i < STEPS.length - 1 && (
                  <div className="flex-1 h-px mx-2" style={i < step ? { backgroundColor: '#2563EB' } : { backgroundColor: '#E2E8F0' }} />
                )}
              </div>
            );
          })}
      ...
      <Card className="p-6">
        {step === 0 && (
          <div className="space-y-4">
            <h3 className="font-semibold text-slate-800">Party Information</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-2">
                <label className="block text-xs font-medium text-slate-700 mb-1">Party / Campaign Name *</label>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">Subdomain *</label>
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-sm text-slate-400 shrink-0">.campaign365.com</span>
                </div>
              </div>
              <div>
        ...
        {step === 1 && (
          <div className="space-y-5">
            <h3 className="font-semibold text-slate-800">Branding & Colors</h3>
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-2">Primary Brand Color</label>
              <div className="flex items-center gap-3">
                <input type="color" value={form.primary_color} onChange={e => set('primary_color', e.target.value)} className="w-12 h-10 rounded-lg border border-slate-200 cursor-pointer" />
                <input value={form.primary_color} onChange={e => set('primary_color', e.target.value)} className="border border-slate-200 rounded-lg px-3 py-2 text-sm w-32 font-mono" />
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-2">Quick Presets</label>
              <div className="flex gap-2 flex-wrap">
                {['#CC0000', '#006400', '#2563EB', '#1D4ED8', '#7C3AED', '#F59E0B', '#000000'].map(c => (
                  <button key={c} onClick={() => set('primary_color', c)}
                    className={`w-8 h-8 rounded-full shadow border-2 hover:scale-110 transition-transform ${form.primary_color === c ? 'border-slate-800 scale-110' : 'border-white'}`}
            ...
            <div>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                ...
                <div className="flex items-center gap-3 p-4">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white font-bold" style={{ backgroundColor: form.primary_color }}>
                    {form.party_name?.[0] || '?'}
                  </div>
                  <span className="font-semibold text-slate-800">{form.party_name || 'Party Name'}</span>
                  <div className="ml-auto px-3 py-1.5 rounded-lg text-white text-xs" style={{ backgroundColor: form.primary_color }}>
                    Sign In
                  </div>
                </div>
              </div>
        ...
        {step === 2 && (
          <div className="space-y-4">
            <h3 className="font-semibold text-slate-800">Select Subscription Plan</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {PLANS.map(plan => (
                <button key={plan.id} onClick={() => set('plan', plan.id)}
                  className={`text-left p-4 rounded-xl border-2 transition-all ${form.plan === plan.id ? 'border-teal-500 bg-teal-50' : 'border-slate-200 hover:border-slate-300'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-bold text-slate-800">{plan.name}</span>
                    <span className="font-bold text-lg" style={{ color: plan.color }}>${plan.price}/mo</span>
                  </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            <h3 className="font-semibold text-slate-800">Admin User Setup</h3>
            <p className="text-sm text-slate-500">This will be the General Secretary / primary admin account.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-2">
        ...
        {step === 4 && (
          <div className="space-y-4">
            <h3 className="font-semibold text-slate-800">Billing Information</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">Billing Contact Name</label>
            ...
            <div className="border border-slate-200 rounded-xl p-4 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">{form.party_name || '(Unnamed Party)'} — {PLANS.find(p => p.id === form.plan)?.name}</span>
                <span className="font-bold text-slate-800">${PLANS.find(p => p.id === form.plan)?.price}/mo</span>
              </div>
              <div className="flex justify-between text-xs text-slate-400">
                <span>Subdomain: {form.subdomain || '(not set)'}.campaign365.com</span>
        ...
        <div className="flex items-center justify-between mt-6 pt-5 border-t border-slate-100">
          <Button variant="ghost" onClick={() => setStep(s => s - 1)} disabled={step === 0} icon={<ArrowLeft size={14} />}>
            Back
          </Button>
          {step < STEPS.length - 1 ? (
            <Button onClick={() => setStep(s => s + 1)} icon={<ArrowRight size={14} />} className="bg-teal-500 border-teal-500 text-white">
              Next: {STEPS[step + 1].label}
            </Button>
          ) : (
            <Button loading={creating} onClick={handleCreate} className="bg-teal-500 border-teal-500 text-white">
              Create Tenant
            </Button>
          )}
        </div>
