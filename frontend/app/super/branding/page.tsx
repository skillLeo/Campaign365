...
export default function BrandingPage() {
  ...
  // Restore saved branding once on mount
  useEffect(() => {
    if (!branding) return;
    const saved = CLIENTS.find(c => c.name === branding.name);
    if (saved) {
      setSelectedClient(saved);
      setPrimaryColor(branding.primary_color ?? saved.color);
      setSecondaryColor(branding.secondary_color ?? saved.secondary);
    } else {
  ...
  const effectiveLogoUrl = uploadRadio === 'url' ? (logoUrlInput || null) : uploadedLogoUrl;

  // Switch client preset — always resets colours to that client's defaults
  const handleClientSelect = useCallback((c: typeof CLIENTS[0]) => {
    setSelectedClient(c);
  ...
  const publishBtnClass =
    'w-full py-2.5 sm:py-3 rounded-xl text-sm font-semibold text-white transition-all hover:opacity-90 flex items-center justify-center gap-2 disabled:opacity-60 px-4';

  return (
    /*
     * IMPORTANT: This wrapper must NOT create a stacking context that competes
     * with the shell's sidebar. No z-index here — just a plain flow container.
     */
    <div className="flex flex-col min-h-screen w-full max-w-7xl mx-auto" style={{ backgroundColor: '#F8FAFC' }}>

      {/*
       * FIX 1 — Top bar z-index
       * ─────────────────────────────────────────────────────────────────────
       * The original code used z-50 which is the SAME level as most sidebar
       * implementations, causing the white bar to paint OVER the sidebar on
       * mobile/tablet and making the "White Label" breadcrumb title appear
       * on top of the sidebar.
       *
       * Fix: drop to z-[29] so the sidebar (typically z-30 / z-40 / z-50)
       * always wins. The bar still floats above normal page content.
       *
       * FIX 2 — No overflow:hidden on this bar
       * ─────────────────────────────────────────────────────────────────────
       * Native <select> dropdown popups are painted by the OS outside the
       * normal DOM flow. They are clipped when a positioned ancestor combines
       * overflow:hidden with a stacking context. We must NOT add overflow
       * hidden here.
       */}
      <div
        className="px-4 sm:px-6 py-3 sm:py-4 bg-white border-b border-slate-100 w-full sticky top-0"
        style={{ zIndex: 29 }}
      >
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0">
          ...
          <div className="flex items-center justify-between w-full sm:w-auto">
            ...
            {/*
             * FIX 3 — Dropdown wrapper
             * ─────────────────────────────────────────────────────────────
             * Keep position:relative for the ChevronDown icon only.
             * Do NOT add overflow:hidden — that is what clips the native
             * select popup on Chrome/Safari on some platforms.
             */}
      ...
      <div className="flex-1 p-3 sm:p-4 lg:p-6 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-5 h-full">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 p-3 sm:p-4 lg:p-5 flex-1 min-h-[400px] flex flex-col">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4 gap-2 sm:gap-0">
                ...
                <div className="relative min-w-[90px]">
                  ...
                  <ChevronDown size={9} className="absolute right-1.5 sm:right-2 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                </div>
              </div>

              {/* Phone mockup — colours driven by live primaryColor state */}
              <div className="flex flex-col items-center flex-1 justify-center">
                <div
                  className="relative rounded-3xl overflow-hidden shadow-xl border-4 border-slate-800 mx-auto"
                  style={{ width: 'clamp(140px,35vw,160px)', height: 'clamp(280px,60vw,300px)', maxWidth: '160px', maxHeight: '300px' }}
                >
                  <div className="absolute inset-0" style={{ backgroundColor: '#F8FAFC' }}>
                    ...
                    <div className="h-8 sm:h-9 flex items-center justify-between px-2 sm:px-3" style={{ backgroundColor: primaryColor }}>
                      <div className="flex items-center gap-1 sm:gap-1.5 min-w-0 flex-1">
                        {effectiveLogoUrl && (
                          <img src={effectiveLogoUrl} alt="logo" className="w-3.5 h-3.5 sm:w-4 sm:h-4 rounded object-contain bg-white/20 flex-shrink-0" />
                        )}
                        <span className="text-white text-xs sm:text-sm font-bold truncate pr-1">{selectedClient.name}</span>
                      </div>
                      <div
                        className="w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center text-white font-bold overflow-hidden flex-shrink-0"
                      ...
                      <div className="grid grid-cols-2 gap-1 sm:gap-1.5 w-full">
                        {[0, 1, 2, 3].map(i => (
                          <div key={i} className="rounded-lg p-1.5 sm:p-2 bg-white border border-slate-100 min-h-[32px]">
                            ...
                            <div className="h-0.5 sm:h-1 bg-slate-100 rounded w-full mb-0.5 mx-auto" />
                            <div className="h-1 sm:h-1.5 rounded w-1/2 mx-auto" style={{ backgroundColor: primaryColor }} />
                          </div>
                        ))}
                      </div>
                      <div
                        className="w-full py-1.5 sm:py-2 rounded-lg text-center text-white text-xs sm:text-sm font-semibold"
          ...
          {/* ── MIDDLE: Colour refs + mini sidebar ─────────────────────────── */}
          <div className="lg:col-span-1 flex-col gap-3 sm:gap-4 order-3 lg:order-2 hidden lg:flex">
            <div className="bg-white rounded-2xl border border-slate-100 p-4 lg:p-5 flex-1">
              <div className="space-y-2 mb-4 lg:mb-5">
                {[
                  { hex: primaryColor,   label: 'Primary'   },
                  { hex: secondaryColor, label: 'Secondary' },
              ...
              <div className="rounded-xl overflow-hidden" style={{ backgroundColor: '#0F172A' }}>
                <div className="px-2.5 sm:px-3 pt-2.5 sm:pt-3 pb-2 sm:pb-2">
                  <div className="flex items-center gap-2 mb-2 sm:mb-3">
                    <div
                      className="w-6 h-6 sm:w-7 sm:h-7 rounded-xl flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
                      style={{ backgroundColor: primaryColor }}
                    >
                      {selectedClient.name.charAt(0)}
                    </div>
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[8px] sm:text-[9px] mt-0.5 text-slate-400">App</p>
                    </div>
                  </div>
                  {[0, 1, 2, 3, 4].map(i => (
                    <div
              ...
              <div className="space-y-3 sm:space-y-4">
                  ...
                  {uploadRadio === 'upload' ? (
                    uploadedLogoUrl ? (
                      <div className="relative border border-slate-200 rounded-xl p-2 sm:p-3 flex items-center gap-2 sm:gap-3 w-full">
                        <img src={uploadedLogoUrl} alt="Logo preview" className="w-8 h-8 sm:w-10 sm:h-10 object-contain rounded-lg border border-slate-100 flex-shrink-0" />
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-4 h-4 rounded flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#2563EB' }}>
